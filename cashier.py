...
ASSETS_PATH = OUTPUT_PATH / Path(r"C:\Users\TaysaMarkAnthony(Stu\Desktop\Arisuu-main\ASSETS\Cashier_assets\frame0")
...
def search_product():
    ...
    for product in products:
        ...
        # Check if the product already exists in the Treeview
        existing_item = None
        for item in tree.get_children():
            if tree.item(item)['values'][0] == product_name:
                existing_item = item
                break

        if existing_item:
            # If the product already exists, increment the quantity by 1
            current_values = tree.item(existing_item, 'values')
            new_quantity = (current_values[2])+ 1  # Increment the stock by 1
            tree.item(existing_item, values=(product_name, product_category, new_quantity, product_price))
            print(f"Updated Product Quantity: {product_name} to {new_quantity}")  # Debug: Confirm update
        else:
            # If the product does not exist, insert it with a quantity of 1
            if search_query in product_name:  # This will be case-sensitive
                tree.insert("", "end", values=(product_name, product_category, 0, product_price))  # Set quantity to 1
                print(f"Inserted Product: {product_name} with quantity 1")  # Debug: Confirm insertion

    update_total_cost()  # Update total cost
    print("Total cost updated.")  # Debug: Confirm total cost updatedef search_product():
    search_query = entry_1.get()  # Get the search query from the entry
    print(f"Search Query: '{search_query}'")  # Debug: Print the search query
    ...
    for product in products:
        ...
        if existing_item:
            # If the product already exists, increment the quantity by 1
            current_values = tree.item(existing_item, 'values')
            new_quantity = int(current_values[2]) + 1  # Increment the stock by 1
            tree.item(existing_item, values=(product_name, product_category, new_quantity, product_price))
            print(f"Updated Product Quantity: {product_name} to {new_quantity}")  # Debug: Confirm update
        else:
            # If the product does not exist, insert it with a quantity of 1
            if search_query in product_name:  # This will be case-sensitive
                tree.insert("", "end", values=(product_name, product_category, 1, product_price))  # Set quantity to 1
                print(f"Inserted Product: {product_name} with quantity 1")  # Debug: Confirm insertion
...
def edit_quantity():
    ...
    current_quantity = int(item_values[2])  # Ensure current stock is an integer

    # Prompt the user for a new stock value
    new_stock_str = simpledialog.askstring("Edit Quantity", f"Current Quantity {current_quantity}\nEnter Quantity")
    ...
    try:
        new_quantity = int(new_stock_str)  # Convert stock to int
    except ValueError:
    ...
    tree.item(selected_item, values=(current_product_name, item_values[1], new_quantity, item_values[3]))

    # Optionally, update the product in the database if you want to keep it in sync
    # db.update_product(current_product_name, current_product_name, item_values[1], new_quantity, item_values[3])  # Update stock

    # Optionally, show a success message
    messagebox.showinfo("Success", f"Quantity for '{current_product_name}' has been updated to {new_quantity}.")
